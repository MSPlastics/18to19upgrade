"""Create the MSP Delivery Slip report on a target Odoo instance.

A customer-facing delivery slip bound to stock.picking. ~95% structurally
identical to the warehouse pick sheet, but:
  - Portrait (6 columns instead of 8) — no Pallets / Weight write-ins
  - Renamed columns: Order Qty -> Ordered Qty, Pick Qty -> Shipped Qty
  - Bottom: Shipper signature/date + Received By signature/date (POD block)
  - Title: "Delivery Slip"
...
Usage:
    python create_msp_delivery_slip.py --target staging         # dry-run
    python create_msp_delivery_slip.py --target staging --commit
    python create_msp_delivery_slip.py --target prod --commit   # AFTER staging confirmed
"""
import argparse
import os
import ssl
...
REPORT_VIEW_KEY = "msp.report_delivery_slip_v1"
REPORT_ACTION_NAME = "Delivery Slip — MSP"

# Self-contained QWEB. Same field-mapping discipline as create_msp_sale_report.py
# and create_msp_pick_sheet.py:
#   - manual qty formatting (no `widget="monetary"`) to dodge wkhtmltopdf
...
QWEB_ARCH = '''<t t-call="web.html_container">
    <t t-foreach="docs" t-as="doc">
        ...
        <div class="page" style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif; color:#111; font-size:9pt;">
            ...
            <!-- TOP LAYOUT: left = company + sold-to/ship-to; right = meta panel -->
            ...
            <table style="width:100%; border-collapse:collapse; margin-bottom:30px;" cellspacing="0">
                <thead>
                    <tr>
                        <th style="width:11%; background-color:#0A182F; color:white; text-align:left;   padding:10px; font-size:8pt; text-transform:uppercase; letter-spacing:0.5px;">MSP PN</th>
...
def upsert_action(call, commit):
    ...
    vals = {
        ...
        "binding_type": "report",
        # Filename: picking name has slashes (WH/OUT/00390); replace so the file
        # lands as WH-OUT-00390.pdf. Evaluated against `object` at render time.
...
def main():
    ...
    if args.commit:
        print("\nDone. The report appears under Print -> Delivery Slip — MSP")
        print("on any Delivery Order. Existing standard reports (Delivery Slip, Picking")
        print("Operations) and the Warehouse Pick Sheet are untouched.")
