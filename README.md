...
- ✅ **Customer Payment Reference end-to-end** (2026-06-02). New Odoo module **`MSPlastics/odoo18:msp_payment_ref`** (depends `account`): adds a **required** Customer Payment Reference to the Register Payment ("Pay") wizard for customer payments (`partner_type='customer'`, hidden for vendor bills), carries it onto `account.payment` via `_create_payment_vals_from_wizard`, mirrors it to the journal-entry Reference (`move.ref`) at create + post, and adds a Customer Payments list column. The `Invoice — MSP` report ([create_msp_invoice.py](workflow/create_msp_invoice.py)) renders it next to each payment line (`Paid on <date>, ref <…>`, pulled from the payment behind each `invoice_payments_widget` entry). Module promoted by **cherry-pick** `19_upgradetest2` → `msp_production` (NOT a blanket promote — staging carries un-promoted msppartialMO / msp_pallet work). Module + report installed/deployed + verified on staging (`32926658`) and **prod** (`16023658`); field names / view xmlids / the wizard hook were pre-verified against live v19 + Odoo 19 source, so the module installed first try on both. Prod key single-use, deleted after.
